...
## Corpus Check
- 47 files · ~25,410 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 578 nodes · 905 edges · 77 communities (35 shown, 42 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3afcad71`
...
2. `Resolved` - 13 edges
3. `CodecKey` - 13 edges
4. `try_serve()` - 12 edges
5. `drive_input()` - 12 edges
6. `build()` - 10 edges
7. `validate_progress()` - 10 edges
8. `Unsafe boundary refactor (deferred until after M3)` - 10 edges
...
- `RequestCtx` --references--> `CodecKey`  [EXTRACTED]
  crates/ngx-compress-module/src/filter.rs → crates/ngx-compress-module/src/worker.rs
- `prefetch_header()` --references--> `Snapshot`  [EXTRACTED]
  crates/ngx-compress-module/src/filter.rs → crates/ngx-compress-module/src/header.rs
- `install_ctx()` --references--> `CodecKey`  [EXTRACTED]
  crates/ngx-compress-module/src/filter.rs → crates/ngx-compress-module/src/worker.rs
- `Plan` --references--> `CodecKey`  [EXTRACTED]
  crates/ngx-compress-module/src/header.rs → crates/ngx-compress-module/src/worker.rs

## Import Cycles
- 1-file cycle: `crates/ngx-compress-codecs/src/identity.rs -> crates/ngx-compress-codecs/src/identity.rs`

## Communities (77 total, 42 thin omitted)
...
Cohesion: 0.10
Nodes (23): AcceptEncoding, BrotliEncoderOperation, BrotliEncoderStateStruct, Brotli, build_state(), map_operation(), ContentCoding, Operation (+15 more)

### Community 3 - "Identity Codec Tests"
Cohesion: 0.17
Nodes (22): add_content_encoding(), core_loc_conf(), handler(), MappedPath, prefetch_request(), register(), ContentCoding, ngx_conf_t (+14 more)
...
Cohesion: 0.11
Nodes (21): CodecError, Compress, copy_into(), Deflate, delta(), derive_state(), FlateCore, FlateStep (+13 more)
...
Cohesion: 0.09
Nodes (41): accept_encoding(), append(), body_filter(), body_filter_inner(), body_filter_with_ctx(), cleanup(), clear_content_length(), compress_chain() (+33 more)
...
### Community 30 - "Result"
Cohesion: 0.24
Nodes (14): acquire(), acquires_a_codec_after_a_successful_reset(), CodecKey, discards_a_codec_when_reset_fails(), Entry, identity_key(), release(), ResetFails (+6 more)
...
Cohesion: 0.14
Nodes (12): c_void, ContentCoding, create(), EncoderOperation, EncoderParameter, map_operation(), SystemBrotli, Drop (+4 more)
...
Cohesion: 0.19
Nodes (21): Resolved, decide(), Plan, AcceptEncoding, Box, ContentCoding, Option, StreamingCodec (+13 more)
...
- **Why does `acquire()` connect `Result` to `6. Local testing plan`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `Resolved` connect `Resolved` to `Self`, `ngx_command_t`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `StreamingCodec` connect `identity.rs` to `Box`, `Progress Contract`, `Resolved`?**
